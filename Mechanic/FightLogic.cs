...
namespace DungeonGame.Mechanic
{
    ...
    public class FightLogic
    {
        ...
        public Fighter Fight(Mag player, List<Fighter> opponents)
        {
            ...
            Fighter b1 = player;
            Fighter b2 = opponent;
            Console.WriteLine("Vítejte v Suboiji!");
        ...
        {
            Draw(combatState.Player, combatState.Opponent);
            WriteMessage(attacker.ReturnLastMessage()); // Zpráva o útoku
            WriteMessage(defender.ReturnLastMessage()); // Zpráva o obraně
        ...
        public Fighter SelectOpponent(List<Fighter> opponents)
        {
            ...
            for (int i = 0; i < opponents.Count; i++)
            {
                ...
                // Kontrola, zda nepřítel již nebyl poražen
    ...
    (__)(__)(_)\_)(____)(_)\_)(__)(__)");
        ...
        private void UseItem(Mag player, Fighter opponent)
        {
            ...
            if (itemIndex == 0)
            {
                ...
                DisplayInventorySectionWithNumbers("Předměty v inventáři", player.Inventory, (item, index) => $"{index + 1}. {item.Name,-20} | {item.Value} zlaťáků");
        ...
        {
            // Check if the player has a HealthPotion in their inventory
            HealthPotion healthPotion = player.Inventory.OfType<HealthPotion>().FirstOrDefault();

            if (healthPotion != null)
            {
                // Zavolání metody pro použití léčivého lektvaru
                healthPotion.Use(player, opponent);

                // Remove the health potion from the player's inventory
                player.Inventory.Remove(healthPotion);

                // Přidání léčivého lektvaru do inventáře hráče (není nutné, pokud chcete, aby byl spotřebován)
                // player.Inventory.Add(healthPotion); // Uncomment this line if you want to keep track of consumed potions in the inventory
            }
            else
            {
        ...
        {
            // Zkontrolujte, zda hráč má meč v inventáři
        ...
        private Item GenerateRandomItem()
        {
            ...
            int randomValue = random.Next(5, 20); // Hodnota předmětu mezi 5 a 20
            bool isSellable = random.Next(0, 2) == 0; // 50% šance, že bude předmět prodán
